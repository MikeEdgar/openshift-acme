...
func (f *Framework) ChangeUser(username string, namespace string) {
	...
	user, err := f.UserClientset().UserV1().Users().Create(ctx, &userv1.User{
	...
	_, err = f.OAuthClientset().OauthV1().OAuthClients().Create(ctx, &oauthv1.OAuthClient{
	...
	token, err := f.OAuthClientset().OauthV1().OAuthAccessTokens().Create(ctx, &oauthv1.OAuthAccessToken{
