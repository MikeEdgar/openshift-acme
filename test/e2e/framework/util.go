...
func CreateTestingNamespace(f *Framework, name string, labels map[string]string) (*v1.Namespace, error) {
	...
	if err := wait.PollImmediate(2*time.Second, 30*time.Second, func() (bool, error) {
		var err error
		got, err = f.KubeClientSet().CoreV1().Namespaces().Create(ctx, namespaceObj, metav1.CreateOptions{})
		if err != nil {
...
func CreateProject(f *Framework, name string, labels map[string]string) (*v1.Namespace, error) {
	...
	_, err := f.ProjectClientset().ProjectV1().ProjectRequests().Create(ctx, &projectapiv1.ProjectRequest{
		ObjectMeta: metav1.ObjectMeta{
			Name:   name,
			Labels: labels,
		},
	...
	err = wait.ExponentialBackoff(retry.DefaultBackoff, func() (bool, error) {
		_, err := f.KubeClientSet().CoreV1().Pods(name).List(ctx, metav1.ListOptions{})
		if err != nil {
...
func DeleteNamespace(f *Framework, ns *v1.Namespace) error {
	...
	ctx, _ := context.WithTimeout(context.Background(), 30*time.Second)
	err := f.KubeAdminClientSet().CoreV1().Namespaces().Delete(ctx, ns.Name, metav1.DeleteOptions{
	...
	err = wait.PollImmediate(1*time.Second, 5*time.Minute, func() (bool, error) {
		...
		_, err := f.KubeAdminClientSet().CoreV1().Namespaces().Get(ctx, ns.Name, metav1.GetOptions{})
		if err != nil {
