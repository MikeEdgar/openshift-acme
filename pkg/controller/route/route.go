...
func (rc *RouteController) syncRouteToSecret(ctx context.Context, key string) error {
	...
	secretObjReadOnly, exists, err := rc.kubeInformersForNamespaces.InformersForOrGlobal(namespace).Core().V1().Secrets().Informer().GetIndexer().GetByKey(namespace + "/" + secretName)
	if err != nil {
		klog.Errorf("Fetching object with key %s from store failed with %v", key, err)
		return err
	}
	...
	if exists {
		klog.V(4).Infof("Secret %q was found in namespace %q", secretName, namespace)
		...
		klog.V(4).Infof("Secret %q was NOT found in namespace %q", secretName, namespace)
